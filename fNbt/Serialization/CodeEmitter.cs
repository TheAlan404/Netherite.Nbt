...
namespace fNbt.Serialization {
    ...
    internal abstract class CodeEmitter {
        public CodeEmitter(Type type, ParameterExpression argTagName, ParameterExpression argValue,
                           NbtCompiler.CallResolver callResolver) {}


        public abstract ParameterExpression[] GetParameters();
        ...
        public abstract Expression HandleIList(string tagName, PropertyInfo property, NullPolicy selfPolicy,
    ...
    internal class SerializeCodeEmitter : CodeEmitter {
        ...
        ParameterExpression argTagName;
        ParameterExpression argValue;
        NbtCompiler.CallResolver callResolver;


        public SerializeCodeEmitter(Type type, ParameterExpression argTagName, ParameterExpression argValue,
                                    NbtCompiler.CallResolver callResolver)
            : base(type, argTagName, argValue, callResolver) {
        ...
        Expression SerializeIList([NotNull] Expression getIListExpr, [NotNull] Type elementType,
            ...
            if (elementType.IsPrimitive || elementType.IsEnum) {
                ...
                if (elementIsIList) {
                    ...
                    // Generate the appropriate enclosing expressions, which choose path depending on NullPolicy
                    handleOneElementExpr = NbtCompiler.MakeNullHandler(varElementValue,
                                                                       getElementExpr,
                                                                       elementPolicy,
        ...
        [NotNull]
        Expression SerializePropertyDirectly([NotNull] PropertyInfo property, [NotNull] string tagName,
            ...
            // generate the appropriate enclosing expressions, depending on NullPolicy
            return NbtCompiler.MakeNullHandler(
                varValue,
                getPropertyExpr,
                selfPolicy,
                makeTagExpr,
        ...
        public override ParameterExpression[] GetParameters() {
            return new[] { varRootTag };
        }
        ...
        public override Expression HandleDirectlyMappedType(string tagName, PropertyInfo property, NullPolicy selfPolicy) {
            return SerializePropertyDirectly(property, tagName, selfPolicy);
        }
        ...
        public override Expression HandleIList(string tagName, PropertyInfo property, NullPolicy selfPolicy,
                                               NullPolicy elementPolicy) {
            Type iListImpl = SerializationUtil.GetGenericInterfaceImpl(property.PropertyType, typeof(IList<>));
            Type elementType = iListImpl.GetGenericArguments()[0];
            Expression getPropertyExpr = Expression.MakeMemberAccess(argValue, property);
            return SerializeIList(getPropertyExpr,
                                  elementType,
